...
class BasicBlock(torch.nn.Module):  # 18/34层网络的残差结构
    ...
    def __init__(self, in_channels, out_channels, stride=1, downsample=None):
        ...
        self.conv1 = torch.nn.Conv2d(in_channels, out_channels, kernel_size=3,
                                     stride=stride, padding=1, bias=False)  # 不使用偏置参数，对于BN来说，使不使用偏置参数都一样
    ...
    def __init__(self, block, blocks_num, num_classes=1000, include_top=True):
        ...
        if self.include_top:
            self.avgpool = torch.nn.AdaptiveAvgPool2d(
                (1, 1))  # 不管图像的长宽尺寸是多少，经过平均池化后都等于长宽1x1的
    ...
    def _make_layer(self, block, channel, block_num, stride=1):  # channel是一个卷积模块中的一个残差结构的输入通道数
        ...
        if stride != 1 or self.in_channels != channel * block.expansion:
            ...
            downsample = torch.nn.Sequential(
                torch.nn.Conv2d(self.in_channels, channel * block.expansion,
                                kernel_size=1, stride=stride, bias=False),
                torch.nn.BatchNorm2d(channel * block.expansion))

        layers = []
        layers.append(block(self.in_channels, channel,
                      downsample=downsample, stride=stride))  # 先把第一个残差结构加到layers
